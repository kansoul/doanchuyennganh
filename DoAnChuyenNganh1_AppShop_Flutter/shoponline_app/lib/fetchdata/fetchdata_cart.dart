...
List<Cart1> parseCart1(String responseBody) {
  final parsed = json.decode(responseBody).cast<Map<String, dynamic>>();
  return parsed.map<Cart1>((json) => Cart1.fromMap(json)).toList();
}

Future<List<Cart1>> fetchCart1(String a) async {
  final response = await http.get(Uri.parse(link[7] + a));
  if (response.statusCode == 200 || response.body != []) {
    return parseCart1(response.body);
  } else {
